...
function showProjectionUrl(url) {
  ...
  urlDiv.innerHTML = `
    ...
    <div class="bg-black text-green-400 p-2 rounded mb-2 font-mono text-sm break-all">${escapeHtml(url)}</div>
    <button onclick="copyProjectionUrl('${escapeHtml(url)}')" class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 w-full mb-2">
      Copier l'URL
    </button>
    ...
    tr.innerHTML = `
      ...
      <td>
        <div class="flex gap-2">
          <button 
            onclick="loadSong('${num.replace(/'/g, "\\'")}','${cat.replace(/'/g, "\\'")}')" 
            class="bg-green-600 hover:bg-green-700 text-white px-3 py-1 rounded text-sm transition-colors"
            aria-label="Charger le chant numéro ${num}">
            CHARGER
          </button>
          <button 
            onclick="editSong('${num.replace(/'/g, "\\'")}','${cat.replace(/'/g, "\\'")}')" 
            class="bg-blue-600 hover:bg-blue-700 text-white px-3 py-1 rounded text-sm transition-colors"
            aria-label="Modifier le chant numéro ${num}">
            MODIFIER
          </button>
          <button 
            onclick="deleteSong('${num.replace(/'/g, "\\'")}','${cat.replace(/'/g, "\\'")}')" 
  ...
  songData.forEach((l, i) => {
    ...
    d.innerHTML = `
      ...
      <button 
        ...
        aria-label="${l.trap ? "Retirer" : "Ajouter"} le piège pour la ligne ${lineNum}">
        PIÈGE
      </button>
    `;
    cont.appendChild(d);
...
function syncNowInternal(val) {
  ...
  const projData = {
    info: currentInfo.n ? `${escapeHtml(currentInfo.c)} - N°${escapeHtml(currentInfo.n)}` : "",
    content: escapedVal ? `<div><i>${escapedVal}</i></div>` : "",
  };
...
function verifyNow() {
  ...
  targetLines.forEach((line) => {
    ...
    line.t.split(/\s+/).forEach((word) => {
      ...
      let cleanI = (input[wordIdx] || "").toLowerCase().replace(/[.,!?;:]/g, "");
      const className = cleanT === cleanI ? "correct" : "wrong";
    ...
    reader.onload = (event) => {
      try {
        ...
        if (
          ...
          if (saveToStorage()) {
            filteredStorage = [...storage];
            showToast(`${imported.length} chant(s) importé(s) avec succès !`, "success");
            if (!document.getElementById("lib-modal").classList.contains("hidden")) {
              renderLibrary();
            }
          }
        }
      } catch (e) {
        showToast("Erreur lors de l'import: Format de fichier invalide", "error");
